...
class AdvDiffBenchmark(PyOP2Benchmark):
    ...
    def __init__(self, np=1, nruns=1, mode='min', message='', version=None, reference=None,
        ...
        # Execute for all combinations of these params
        ...
        super(AdvDiffBenchmark, self).__init__(params)
        self.primed = []
    ...
    def dry_run(self, version, meshsize):
        if version not in self.primed:
            self.log("Performing dry run of %s using mesh size %sx%s"
                     % (version, meshsize, meshsize))
    ...
    def plot_result(self, format='svg'):
        ...
        for fig, pl in zip(('linear', 'semilogx'), (pylab.plot, pylab.semilogx)):
            self._plot('runtime_' + fig, pl, lambda x: x, 'upper left',
                       'Overall runtime in seconds', title, format)
